...
(function () {
	...
	var _isArray = Array.isArray || function (arr) {
		return _isObject(arr) && _isNumber(arr.length) && _toString(arr) === "[object Array]";
	}
		...
		(function iterator () {
			...
			flow(tasks, function (k, cb) {
				...
				if (_isArray(target)) {
					...
					for (; i < fin; i++) {
						if (!_hasOwnProperty(results, target[i]))
							return _later(cb);
					}
