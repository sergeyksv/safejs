...
!(function () {
	...
	var _isMap = function (map) {
		return _toString.call(map) === '[object Map]';
	};
	...
	var _arEach = /*_Array.prototype.forEach ? _Function.prototype.call.bind(_Array.prototype.forEach) :*/ function (arr, task) {
	...
	var _iterator = function (obj) {
		...
		if (_isArray(obj)) {
			return {
				next: function () {
					++i;
					return i < obj.length ? {value: obj[i], key: i, done: false} : {done: true};
				}
			};
		} else if (_iteratorSymbol && obj[_iteratorSymbol]) {
			...
			return {
				next: function () {
					...
					if (item.done)
						return {done: true};
					return isMap ? {value: item.value[1], key: item.value[0], done: false} : {value: item.value, key: ++i, done: false};
				}
			};
		} else {
			keys = _keys(obj);
			return {
				next: function () {
					var k = keys[++i];
					return i < keys.length ? {value: obj[k], key: k, done: false} : {done: true};
				}
			};
		}
	};
	...
	_queue.prototype.resume = function () {
		...
		for (; w < self.concurrency && iterator.next().done === false; w++) {
			self.__execute();
		}
	};
