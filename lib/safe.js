	...
	_createClass(_priorQ, [{
		...
		value: function __insert(data, prior, callback) {
			...
			if (!this.__processingScheduled) {
				this.__processingScheduled = true;
				_back(function () {
					_this9.__processingScheduled = false;
					_this9.__execute();
				});
			}
		}
	...
	_createClass(_cargoQ, [{
		...
		value: function __execute() {
			...
			var _loop3 = function _loop3() {
				...
				(_workersList = _this12.__workersList).push.apply(_workersList, _toConsumableArray(tasks));
				...
				_run_once(function (cb) {
					...
					tasks.forEach(function (task) {
						var index = _this12.__workersList.indexOf(task);
						if (index >= 0) {
					...
					if (_this12.__workers <= _this12.concurrency - _this12.buffer) {
						_this12.unsaturated();
					}
