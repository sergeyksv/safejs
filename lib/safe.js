(function () {
	...
	var isObject = function (un) {
		return typeof un === "object";
	}
	...
	safe.args = function() {
		...
		var rest = this && parseInt(this) > 0 ? parseInt(this) : 0;
		...
		safe[flow](keys(arr), function (key, cb) {
			safe.run(arr[key], safe.sure_result(cb, function () {
				if (arguments.length > 1) {
					results[key] = safe.args.apply(null, arguments);
				} else
	...
	safe.sure = safe.trap_sure = function (callback, fn) {
		if (fn == undefined || !isFunction(callback))
			throw new Error("Exactly two arguments are required")
		...
		var iterator = function (err) {
			...
			safe.each(tasks, function (k, cb) {
				...
				safe.run(function (cb) {
					...
					if (arguments.length > 1) {
						results[k] = safe.args.apply(null, arguments);
					} else
