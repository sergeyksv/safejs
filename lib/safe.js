...
!(function () {
	...
	var _wrapArgs = function (fn) {
		...
		return function () {
			var i = -1,
				len = rest ? len = arguments.length === 0 ? 0 : arguments.length - 1 : arguments.length,
				args = _Array(len);
			...
			if (rest) {
				fn.call(this, args, arguments[len]);
			} else {
				fn.call(this, args);
			}
		};
	...
	var _sure_result = function (callback, fn) {
		...
		return function (err) {
			...
			var result = _catcher(fn, this, _argToArr.apply(1, arguments), function (er) {
				err = er;
			});
	...
	var _sure_spread = function (callback, fn) {
		...
		return _wrapArgs(function (args) {
			...
			_catcher(fn, this, args[1], callback);
		});
	};
	...
	_queue.prototype.resume = function () {
		...
		while (++w <= self.concurrency && next() !== null) {
			self.__execute();
		}
	};
