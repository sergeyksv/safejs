...
(function () {
	...
	var _mapLimit = function (flow, arr, fn, callback) {
		callback = _once(callback || _noop);
		...
		flow(arr, function (item, cb) {
			...
			fn(item, _sure_result(cb, function (res) {
				result[i] = res;
			}));
		}, _sure(callback, function () {
			callback(null, result);
	...
	safe.prototype.mapLimit = function (arr, limit, fn, callback) {
		_mapLimit(_eachLimit(limit), arr, fn, callback);
	}

	safe.prototype.map = function (arr, fn, callback) {
		_mapLimit(_eachLimit(arr.length), arr, fn, callback);
	}

	safe.prototype.mapSeries = function (arr, fn, callback) {
		_mapLimit(_eachSeries, arr, fn, callback);
	}
