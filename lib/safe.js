...
var isAsyncFunction = function isAsyncFunction(f) {
...
var _asyncify = function _asyncify(fn) {
	return function () {
		var _this3 = this;
...
var _tryEach = function _tryEach(obj, callback) {
	if (!_isObject(obj, callback)) {
		return _throwError(_typedErrors[0], callback);
	}
	...
	_createClass(_queue, [{
		...
		value: function __insert(data, pos, callback) {
			...
			if (!this.__processingScheduled) {
				...
				_back(function () {
					_this5.__processingScheduled = false;
					_this5.__execute();
				});
			}
		}
		...
		value: function __execute() {
			...
			var _loop2 = function _loop2() {
				...
				_run_once(function (cb) {
					...
					var index = _this6.__workersList.indexOf(task);
					if (index >= 0) {
					...
					if (_this6.__workers <= _this6.concurrency - _this6.buffer) {
						_this6.unsaturated();
					}
	...
	_createClass(_priorQ, [{
		...
		value: function __insert(data, prior, callback) {
			...
			if (!this.__processingScheduled) {
				this.__processingScheduled = true;
				_back(function () {
					_this10.__processingScheduled = false;
					_this10.__execute();
				});
			}
		}
...
var _cargoQ = function (_queue4) {
	...
	function _cargoQ(worker, payload) {
		...
		var _this12 = _possibleConstructorReturn(this, (_cargoQ.__proto__ || Object.getPrototypeOf(_cargoQ)).call(this, worker, 1, 'Cargo'));
	...
	_createClass(_cargoQ, [{
		...
		value: function __execute() {
			...
			var _loop3 = function _loop3() {
				...
				var tasks = _this13.tasks.splice(0, _this13.payload);
				(_workersList = _this13.__workersList).push.apply(_workersList, _toConsumableArray(tasks));

				if (_this13.tasks.length === 0) _this13.empty();

				var data = tasks.map(_byData);

				_this13.__workers++;

				if (_this13.__workers === _this13.concurrency) _this13.saturated();

				_run_once(function (cb) {
					return _this13.__worker.call(null, data, cb);
				}, function () {
					...
					tasks.forEach(function (task) {
						var index = _this13.__workersList.indexOf(task);
						if (index >= 0) {
							_this13.__workersList.splice(index, 1);
						}

						task.callback.apply(task, args);
					});

					if (_this13.__workers <= _this13.concurrency - _this13.buffer) {
						_this13.unsaturated();
					}

					if (_this13.idle()) _this13.drain();
