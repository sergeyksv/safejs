(function () {
	...
	var later = (typeof setImmediate === "undefined") ? (typeof process === "undefined" ? function (cb) {setTimeout(cb,0)} : process.nextTick) : function (cb) { setImmediate (cb) };
	...
	safe.auto = function (obj, callback) {
		...
		var iterator = function () {
			...
			safe.each(tasks, function (k, cb) {
				...
				safe.run(function (cb) {
					task(cb, res);
				}, safe.sure_result(iterator || callback, function () {
					qnt--;
					var args = Array.prototype.slice.call(arguments, 0);
					if (args.length <= 1)
						args = args[0]; // behavior compatible with async
