...
!(function () {
	...
	var _executeSeries = function (chain, callback) {
		if (!_isIterable(chain)) {
			return _throwError(_errors[0], callback);
		}
	...
	safe.each = safe.forEach = function (arr, fn, callback) {
		if (!_isArray(arr)) {
			return _throwError(_errors[1], callback);
		}
	...
	safe.eachSeries = safe.forEachSeries = function (arr, fn, callback) {
		if (!_isArray(arr)) {
			return _throwError("Array is required", callback);
		}
	...
	safe.filter = function (arr, fn, callback) {
		if (!_isArray(arr)) {
			return _throwError(_errors[1], callback);
		}
	...
	safe.filterLimit = function (arr, limit, fn, callback) {
		if (!_isArray(arr)) {
			return _throwError(_errors[1], callback);
		}
	...
	safe.filterSeries = function (arr, fn, callback) {
		if (!_isArray(arr)) {
			return _throwError(_errors[1], callback);
		}
