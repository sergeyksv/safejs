...
var UNDEFINED = 'undefined',
    ...
    _isPromiseLike = function _isPromiseLike(p) {
	...
	(function task(err) {
		...
		if (err) {
			...
			(function () {
				...
				if (item === null) {
					callback.apply(_this5, _arguments);
				} else {
					_run_once(function (cb) {
						args.push(cb);
						return item.value.apply(_this5, args);
					}, task);
				}
			})();
		}
...
var wrap2to3 = function wrap2to3(fn) {
...
var _apply = function _apply(fn) {
	...
	return function () {
		...
		return fn.apply(_this6, args.concat(args2));
	};
};
	...
	return function (fns) {
		var task = function task() {
			...
			flow(fns, function (fn, key, cb) {
				return fn.apply(_this7, args2.concat(cb));
			}, callback);
		};
...
_seriesQ.prototype.__execute = _priorQ.prototype.__execute = function () {
	...
	if (!this.paused && this.__workers < this.concurrency && this.tasks.length !== 0) {
		(function () {
			var task = _this8.tasks.shift();
			_this8.__workersList.push(task);

			if (_this8.tasks.length === 0) _this8.empty();

			++_this8.__workers;

			if (_this8.__workers === _this8.concurrency) _this8.saturated();
			...
			var cb = _only_once(function () {
				...
				_arEach(_this8.__workersList, function (worker, index) {
					if (worker === task) {
						_this8.__workersList.splice(index, 1);
						return false;
					}
				});
				...
				if (args[0]) {
					_this8.error(args[0], task.data);
				}

				if (_this8.idle()) _this8.drain();

				_this8.__execute();
			});

			_run_once(function (cb) {
				return _this8.__worker.call(task, task.data, cb);
			}, cb);
		})();
	}
};
	...
	if (!this.paused && this.__workers < this.concurrency && this.tasks.length !== 0) {
		(function () {
			...
			var cb = _only_once(function () {
				...
				_arEach(tasks, function (task) {
					_arEach(_this9.__workersList, function (worker, index) {
						if (worker === task) {
			...
			_run_once(function (cb) {
				return _this9.__worker.call(null, data, cb);
			}, cb);
		})();
	}
};
...
var safe = {
	...
	reflectAll: function reflectAll(tasks) {
		...
		if (_isArray(tasks)) {
			...
			result = {};
			_arEach(_keys(tasks), function (key) {
