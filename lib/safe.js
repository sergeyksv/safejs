(function () {
	...
	var controwFlow = function (arr, limit, callback) {
		var results = isArray(arr) ? new Array(arr.length) : {};

		safe.eachLimit(keys(arr), limit, function (i, cb) {
	...
	var reduce = function (arr, memo, fn,  callback, direction) {
		...
		function execute (err, memo) {
			...
			safe.run(function (cb) {
				iter++;
				var task = direction ? arr[iter - 1] : arr[arr.length - iter];
	...
	safe.series = function (arr, callback) {
		controwFlow(arr, 1, callback);
	}

	safe.parallel = function (arr, callback) {
		controwFlow(arr, null, callback);
	}

	safe.parallelLimit = function (arr, limit, callback) {
		controwFlow(arr, limit, callback);
	}
	...
	var map = function (type, arr, limit, fn, callback) {
		callback = once((callback || function () {}), 1);

		var results = type === "concat" ? [] : new Array(arr.length);

		safe.eachLimit(keys(arr), limit, function (k, cb) {
			fn(arr[k], safe.sure_result(cb, function (res) {
				if (type === "sort")
					results[k] = {x:res, k:k};
				else if (type === "concat")
		...
		}, safe.sure_result(callback, function () {
			if (type !== "sort")
				return results;

			var res = new Array(arr.length);
			forEach(results.sort(function (a,b) {
