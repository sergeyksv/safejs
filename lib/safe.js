...
(function () {
	...
	var _argToArr = function() {
		...
		for (var i = rest, l = arguments.length; i < l; i += 1)
	...
	var _auto = function (obj, callback) {
		...
		_forEach(tasks, function (key) {
			if (_isArray(obj[key])){
				for (var i = 0, targer = obj[key], deps, l = targer.length - 1; i < l; i += 1) {
					deps = obj[targer[i]];
	...
	_queue.prototype.resume = function () {
		...
		for (var w = 1; w <= self.concurrency; w += 1)
	...
	_queue.prototype.__insert = function (data, pos, callback) {
		...
		for (var i = 0; self.concurrency >= self.__workers + i && i < arlen && !self.paused; i += 1) {
			_later(function () { self.__execute(); });
		}
	}
