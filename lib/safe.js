...
(function () {
	...
	var _auto = function (obj, callback) {
		...
		(function iterator () {
			...
			flow(tasks, function (k, cb) {
				...
				_run(function (cb) {
					task(cb, results);
				}, _sure(function (err) { callback(err, results); callback = _noop; }, function () {
					qnt--;
	...
	var _queue = function (worker, concurrency, isprior) {
		var workers = 0,
			tasks = [],
			self = this;
		...
		function execute() {
			if (!self.paused && workers < self.concurrency && tasks.length) {
		...
		function insert(data, pos, callback) {
			...
			var arr = new Array(data.length), tlen = tasks.length;
			_forEach(data, function (task, i) {
				arr[i] = {
					data: task,
					priority: pos,
				...
				if (self.saturated && tlen + i > self.concurrency) {
					self.saturated();
				}
			});
			...
			if (tlen) {
				if (isprior) {
					var firstidx = tlen ? tasks[0].priority : 0;
					var lastidx = tlen ? tasks[tlen - 1].priority : 0;
					...
					if (firstidx >= pos && pos < lastidx) {
						tasks.sort(function (a, b) { // reverse sort
			...
			for (var i = 0; self.concurrency >= workers + i && i < arr.length && !self.paused; i++) {
				_later(execute);
			}
		}
		...
		if (isprior) {
			self.push = function (data, priority, callback) {
				insert(data, priority, callback);
			}
		} else {
	...
	safe.prototype.priorityQueue = function (worker, threads) {
		return new _queue(worker, threads, 1);
	}
