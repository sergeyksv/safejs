...
(function () {
	...
	var _queue = function (worker, threads, isprior) {
		...
		function insert(data, pos, callback) {
			...
			if (tasks.length) {
				if (isprior) {
					var firstidx = tasks.length ? tasks[0].priority : 0;
					var lastidx = tasks.length ? tasks[tasks.length - 1].priority : 0;
			...
			for (var i = 0; threads >= workers + i && i < arr.length && !self.paused; i++) {
				_later(execute);
			}
		}
