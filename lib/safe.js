...
!(function () {
	...
	var _arEach = /*_Array.prototype.forEach ? _Function.prototype.call.bind(_Array.prototype.forEach) :*/ function (arr, fn) {
		var next = _iterator(arr),
			key;

		while ((key = next()) !== null) {
			if (fn(arr[key], key, arr) === false) {
				break;
			}
		}
	};
	...
	var _executeSeries = function (chain, callback) {
		...
		(function iterator(err) {
			...
			_run(function (cb) {
				args.push(cb);
				return chain[key].apply(this, args);
			}, iterator);
		})();
	};
