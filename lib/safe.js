...
!(function () {
	...
	var _run = function (fn, callback) {
		...
		var res = _catcher(fn, this, callback);
	...
	var _reduce = function (arr, memo, fn, callback, direction) {
		...
		(function iterator(err, memo) {
			...
			_run(function (cb) {
				if (direction)
					return fn(memo, (arr[iter++]), cb);

				return fn(memo, (arr[len - (++iter)]), cb);
			}, iterator);
		})(null, memo);
	};
	...
	var _eachOf = function (flow, obj, fn, callback) {
		...
		flow(_keys(obj), function (key, cb) {
			if (len === 3)
	...
	safe.concat = function (obj, fn, callback) {
		_concat(_eachLimit(_size(obj)), obj, fn, callback);
	};

	safe.concatLimit = function (obj, limit, fn, callback) {
		_concat(_eachLimit(limit), obj, fn, callback);
	};

	safe.concatSeries = function (obj, fn, callback) {
		_concat(_eachSeries, obj, fn, callback);
	};
	...
	safe.sortBy = function (arr, fn, callback) {
		...
		_sortBy(_eachLimit(arr.length), arr, fn, callback);
	};
	...
	safe.filterLimit = function (obj, limit, fn, callback) {
		_filter(_eachLimit(limit), 1, obj, fn, callback);
	};
	...
	safe.rejectLimit = function (obj, limit, fn, callback) {
		_filter(_eachLimit(limit), 0, obj, fn, callback);
	};
